...
class BaseBallotClass(BaseModel, ABC):
    ...
    # def __init__(self, poll_id: uuid.UUID, data: dict, github_id: int):
    #     super().__init__(poll_id=poll_id, data=data, id=ballot_id(poll_id, github_id))
...
def get_public_polls() -> list:
    poll_directory = Path(os.environ["POLL_DIRECTORY"])
    return [poll for poll in poll_directory.iterdir() if poll.stem != "private"]
